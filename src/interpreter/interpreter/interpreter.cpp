...
namespace interpreter {
    ...
    void Interpreter::run() {
        RuntimeResult rr = this->evaluate(this->scope, this->ast, false);
        this->error = rr.error;
    }

    RuntimeResult Interpreter::evaluate(const std::shared_ptr<Scope> scope, const std::shared_ptr<parser::Statement> stmt, const bool in_loop) {
        ...
        case BlockStmt: {
            ...
            for (const std::shared_ptr<parser::Statement> &stmt : block->body) {
                if (stmt->node_type == BreakStmt) {
                    if (!in_loop) {
                        return RuntimeResult(nullptr, std::make_shared<errors::LoopError>(errors::LoopError(stmt->ctx, "breaking outside of a loop is illegal")));
                    }

                    return RuntimeResult(nullptr, nullptr, true);
                }

                if (stmt->node_type == ContinueStmt) {
                    if (!in_loop) {
                        return RuntimeResult(nullptr, std::make_shared<errors::LoopError>(errors::LoopError(stmt->ctx, "continuing outside of a loop is illegal")));
                    }

                    return RuntimeResult(nullptr, nullptr, false, true);
                }

                RuntimeResult res = this->evaluate(scope, stmt, in_loop);
                if (res.error != nullptr) {
                    return res;
                }

                if (res.breaking || res.continuing) {
                    return res;
                }
            }
        ...
        case VariableDeclarationStmt: {
            ...
            RuntimeResult data_type = this->evaluate(scope, variable_declaration->data_type, in_loop);
            ...
            RuntimeResult value = this->evaluate(scope, variable_declaration->value, in_loop);
        ...
        case IfElseStmt: {
            ...
            while (if_else != nullptr) {
                RuntimeResult condition = this->evaluate(scope, if_else->condition, in_loop);
                ...
                if (boolean->value) {
                    RuntimeResult body = this->evaluate(std::make_shared<Scope>(Scope(scope)), if_else->body, in_loop);
                    return body;
                }
        ...
        case WhileStmt: {
            ...
            while (true) {
                RuntimeResult condition = this->evaluate(scope, while_->condition, in_loop);
                if (condition.error != nullptr) {
                    return condition;
                }
                ...
                loop_scope->clear();
                RuntimeResult body = this->evaluate(loop_scope, while_->body, true);
        ...
        case ForV1Stmt: {
            ...
            RuntimeResult initialization = this->evaluate(child_scope, for_v1->initialization, in_loop);
            ...
            while (true) {
                RuntimeResult condition = this->evaluate(child_scope, for_v1->condition, in_loop);
                if (condition.error != nullptr) {
                    return condition;
                }
                ...
                loop_scope->clear();
                RuntimeResult body = this->evaluate(loop_scope, for_v1->body, true);
                if (body.error != nullptr) {
                    return body;
                }
                ...
                RuntimeResult increment = this->evaluate(child_scope, for_v1->increment, in_loop);
        ...
        case AssignmentExpr: {
            ...
            case IdentifierExpr: {
                RuntimeResult assignee = this->evaluate(scope, assignment->assignee, in_loop);
        ...
        case BinaryExpr: {
            ...
            RuntimeResult lhs = this->evaluate(scope, binary->lhs, in_loop);
            if (lhs.error != nullptr) {
                return lhs;
            }

            RuntimeResult rhs = this->evaluate(scope, binary->rhs, in_loop);
        ...
        case UnaryExpr: {
            ...
            RuntimeResult value = this->evaluate(scope, unary->value, in_loop);
            if (value.error != nullptr) {
                return value;
            }
        ...
        case CallExpr: {
            ...
            RuntimeResult callee = this->evaluate(scope, call->callee, in_loop);
            ...
            for (const std::shared_ptr<parser::Expression> &arg : call->arguments) {
                RuntimeResult argument = this->evaluate(scope, arg, in_loop);
        ...
        case MemberExpr: {
            ...
            RuntimeResult parent = this->evaluate(scope, member->parent, in_loop);
